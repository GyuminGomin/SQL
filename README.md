...
- 애플리케이션의 트랜잭션 유형 (CRUD)을 파악한다.
- 합계 및 정산 등을 수행하는 반정규화를 수행한다.
- 기본키와 외래키, 슈퍼타입과 서브타입 등을 조정한다.
- 성능관점에서 데이터 모델을 검증하고 확인한다.

### 순수 관계 연산자
`관계형 데이터베이스에 적용할 수 있도록 개발한 관계 연산자를 의미함.`
- SELECT, PROJECT, JOIN, DIVIDE가 있음.
...
`아래의 내용은 주식별자의 어떤 특징을 설명한 것인가?`
```
학생의 학번으로 고유한 구조를 표현할 수 있다. 하지만 ABC 대학교의 학생 엔터티의 주
식별자를 학번과 입학일자로 해서 잘못된 모델링을 했다.
```
- 최소성 : 주식별자는 최소성을 만족하는 키이다.
    - cf. 대표성 : 주식별자는 엔터티를 대표할 수 있어야 한다.
    - cf. 유일성 : 주식별자는 엔터티의 인스턴스를 유일하게 식별한다.
    - cf. 불변성 : 주식별자는 자주 변경되지 않아야 한다.


`SELECT NVL(COUNT(*), 9999) FROM TABLE WHERE 1=2 의 결괏값은?`
- 0 -> 집계 함수에서 count(*) 함수는 조건절이 거짓 일 때 0을 반환

`다음 중 인덱스에 대한 설명으로 올바르지 않은 것은?`
- 인덱스는 순차 인덱스, 결합 인덱스, 비트맵, 클러스터, 해시 인덱스가 있다.
- VARCHAR, CHAR, DATE, NUMBER 모두 인덱스 생성이 가능하다.
- 파티션 테이블은 파티션 키에 대해 인덱스를 생성할 수 없다. (x) -> 파티션 키에 생성한 인덱스를 로컬 파티션 인덱스, 파티션 키와 다른 칼럼으로 생성한 인덱스를 글로벌 파티션 인덱스라 함.
